...
public:
        ...
        : Layer("Example"), m_Camera(-1.6f, 1.6f, -0.9f, 0.9f), m_CameraPosition(0.0f)
    {
        ...
        m_Shader.reset(COLA::Shader::Create(vertexSrc, fragmentSrc));
        ...
        m_FlatColorShader.reset(COLA::Shader::Create(flatColorShaderVertexSrc, flatColorShaderFragmentSrc));
        ...
        m_TextureShader.reset(COLA::Shader::Create(textureShaderVertexSrc, textureShaderFragmentSrc));

        m_Texture = COLA::Texture2D::Create("assets/textures/Checkerboard.png");

        std::dynamic_pointer_cast<COLA::OpenGLShader>(m_TextureShader)->Bind();
        std::dynamic_pointer_cast<COLA::OpenGLShader>(m_TextureShader)->UploadUniformInt("u_Texture", 0);
    }
    void OnUpdate(COLA::Timestep ts) override
    {
        if (COLA::Input::IsKeyPressed(COLA_KEY_LEFT))
            m_CameraPosition.x -= m_CameraMoveSpeed * ts;
        else if (COLA::Input::IsKeyPressed(COLA_KEY_RIGHT))
            m_CameraPosition.x += m_CameraMoveSpeed * ts;
        if (COLA::Input::IsKeyPressed(COLA_KEY_UP))
            m_CameraPosition.y += m_CameraMoveSpeed * ts;
        else if (COLA::Input::IsKeyPressed(COLA_KEY_DOWN))
            m_CameraPosition.y -= m_CameraMoveSpeed * ts;
        if (COLA::Input::IsKeyPressed(COLA_KEY_A))
            m_CameraRotation += m_CameraRotationSpeed * ts;
        if (COLA::Input::IsKeyPressed(COLA_KEY_D))
            m_CameraRotation -= m_CameraRotationSpeed * ts;
        COLA::RenderCommand::SetClearColor({ 0.1f, 0.1f, 0.1f, 1 });
        COLA::RenderCommand::Clear();
        m_Camera.SetPosition(m_CameraPosition);
        m_Camera.SetRotation(m_CameraRotation);
        COLA::Renderer::BeginScene(m_Camera);
        ...
        m_Texture->Bind();
        COLA::Renderer::Submit(m_TextureShader, m_SquareVA, glm::scale(glm::mat4(1.0f), glm::vec3(1.5f)));
...
private:
    ...
    COLA::Ref<COLA::Shader> m_FlatColorShader, m_TextureShader;
    COLA::Ref<COLA::VertexArray> m_SquareVA;

    COLA::Ref<COLA::Texture2D> m_Texture;
    COLA::OrthographicCamera m_Camera;
